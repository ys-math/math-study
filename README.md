...
| `/bib <topic> <citation>` | Files a reference you supply into `tex/<topic>/bibliography.tex` per `docs/bib-convention.md`, creating that file and `\input`ing it from `main.tex` before the colophon when the topic has none. It looks nothing up and inserts no `\cite{}` — the citation is yours, and so is the sentence that cites it |
