...
| Hook | Event | Refuses |
| --- | --- | --- |
| `guard-bash.sh` | `PreToolUse(Bash)` | `git add -A` / `git add .`; force-push including `--force-with-lease`; `git clean` with no pathspec, unless it is a dry run; `git restore` whose pathspec is `.` or `:/` |
