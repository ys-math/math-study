...
- **The diff falls inside a `<!-- BEGIN … -->` / `<!-- END … -->` block in
  `README.md` whose generator is not also in the diff.** Those are generated by
  `update-readme.yml`; the edit will be overwritten on the next push. Prose
  *outside* the blocks is fine, and so is the commit that adds a generator
  alongside the first copy of its block — nothing overwrites that one yet.
