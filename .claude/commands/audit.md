...
allowed-tools: Read, Glob, Grep, Bash(python -m unittest:*), Bash(git rev-parse:*), Bash(date:*), Write(reviews/**)
---
...
markers — and write a report to `reviews/audit.md`. That directory is
gitignored: the report is scaffolding, not a record.
...
**This command never edits what it audits.** It reports; applying a fix is a
separate instruction from the user. `allowed-tools` enforces that — the only
writable path is `reviews/`.

**It never audits `tex/**`.** The mathematics belongs to the repo owner and to
`/review-notes`. A note that contradicts itself is not this command's business.
...
Ignore any previous `reviews/audit.md`. Audit fresh and overwrite it, carrying
nothing over — including the state of its checkboxes.

## This is not a `/loop` target

`/watch-ci` ends with a machine-readable state because the thing it watches
changes on its own. This one does not: the report is a function of prose that
only a human edits, so a second unattended run reproduces the first exactly.
Run it after the system changes, not on a schedule — and there is deliberately
no `audit: ...` line for a loop to read.

## The report

English, at `reviews/audit.md`, overwritten each run. Findings grouped by
...
- [ ] **1** Contradiction — `/label` declares English output, then prints `(空)`
      [`label.md:118`](../.claude/commands/label.md#L118) [High]
...
[`label.md:118`](../.claude/commands/label.md#L118)
...
the report sits in `reviews/`, so paths are relative and the `#L<n>` fragment
opens the file at the line in a Markdown preview. Keep the link text in
backticks so it still reads as a path. A **Fix** describes a change that is not
in the file yet, so it names its target in a plain code span, not a link.

Keep every heading, writing `None.` under the categories with nothing in them —
the absences are the part a reader trusts. Write every checkbox unticked; they
are the owner's own tracking and a re-run resets them.
...
```
reviews/audit.md written (mechanical: <n> pass / <category> <count> / …)
...
Then stop. Do not fix anything, and do not commit: `reviews/` is gitignored and
