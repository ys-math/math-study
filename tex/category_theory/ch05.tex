...
$F \dashv G$が存在するとする.\ $(\eta, \varepsilon) \coloneq F \dashv G$とすると$\varepsilon F \circ F \eta = \id_F$かつ$G \varepsilon \circ \eta G = \id_G$が成り立つ.\ $X \in \mathscr{C},\ Y \in \mathscr{D}$に対して$\Phi_{(X,Y)} \colon \Mor_{\mathscr{D}}(F(X), Y) \to \Mor_{\mathscr{C}}(X, G(Y))$を$\Phi(f) \coloneq G(f) \circ \eta_X$で定め$\Psi_{(X,Y)} \colon \Mor_{\mathscr{C}}(X, G(Y)) \to \Mor_{\mathscr{D}}(F(X), Y)$を$\Psi_{(X,Y)}(g) \coloneq \varepsilon_Y \circ F(g)$で定める.

\end{proof}

\begin{definition} \label{def: Adjunction.limitFunctor}
