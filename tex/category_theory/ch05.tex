...
$\mathscr{C},\ \mathscr{D},\ \mathscr{E}$を圏とする.\
$F \colon \mathscr{C} \to \mathscr{D}$と$G \colon \mathscr{D} \to \mathscr{E}$を共変関手とする.\
このとき\term{ごうせいかんしゅ}{合成関手}{composite functor}$GF \colon \mathscr{C} \to \mathscr{D}$が次のようにして定まる.
\begin{itemize}
...
$\mathscr{C},\ \mathscr{D},\ \mathscr{E}$を圏,\ $F,\ G \in \mathscr{D}^{\mathscr{C}}$,\ $H \in \mathscr{E}^{\mathscr{D}}$,\ $\eta \colon F \rightarrow G$とする.\
自然変換$H\eta \colon HF \Rightarrow HG$を$X \in \mathscr{C}$に対して$(H\eta)_X \coloneq H(\eta_X)$として定める.\
$H \eta$を$H$による$\eta$への\term{ひだりひげけつごう}{左ヒゲ結合}{left whiskering}という.
\end{definition}

\begin{definition} \label{def: rightWhiskering}
$\mathscr{C},\ \mathscr{D},\ \mathscr{E}$を圏,\ $F,\ G \in \mathscr{E}^{\mathscr{D}}$,\ $H \in \mathscr{D}^{\mathscr{C}}$,\ $\eta \colon F \rightarrow G$とする.\
自然変換$\eta	H \colon FH \Rightarrow GH$を$X \in \mathscr{C}$に対して$(\eta H)_X \coloneq \eta_{H(X)}$として定める.\
$\eta H$を$H$による$\eta$への\term{みぎひげけつごう}{右ヒゲ結合}{right whiskering}という.
\end{definition}

\begin{definition} \label{def: Adjunction}
$\mathscr{C},\ \mathscr{D}$を圏とする.\
$F \colon \mathscr{C} \to \mathscr{D}$と$G \colon \mathscr{D} \to \mathscr{C}$を共変関手とする.\
自然変換$\eta \colon \id_{\mathscr{C}} \Rightarrow GF$と自然変換$\varepsilon \colon FG \Rightarrow \id_{\mathscr{D}}$で$\varepsilon F \circ F \eta = \id_{F}$かつ$G \varepsilon \circ \eta G = \id_{G}$を満たすものが存在するとき$F \dashv G \coloneq (\eta, \varepsilon)$としこれを\term{ずいはん}{随伴}{adjunction}という.\
随伴$F \dashv G$が存在するとき$F$は$G$の\term{ひだりずいはん}{左随伴}{left adjoint},\ $G$は$F$の\term{みぎずいはん}{右随伴}{right adjoint}であるといい$\eta$を\term{たんい}{単位}{unit},\ $\varepsilon$を\term{よたんい}{余単位}{counit}という.
...
$\mathscr{C}, \mathscr{D}$を圏とする.\
\term{せきけん}{積圏}{product category}$\mathscr{C} \times \mathscr{D}$は$A \in \Ob(\mathscr{C}), B \in \Ob(\mathscr{D})$の組$(A, B)$を対象とし$f \in \Mor(\mathscr{C}), g \in \Mor(\mathscr{D})$の組$(f, g)$を射とする.\
合成は$(f_2, g_2) \circ (f_1, g_1) \coloneq (f_2 \circ f_1, g_2 \circ g_1)$で定める.\
恒等射は$\id_{(A,B)} \coloneq (\id_A, \id_B)$で定める.
\end{definition}

\begin{definition} \label{def: Bifunctor}
$\mathscr{C}, \mathscr{D}, \mathscr{E}$を圏とする.\
$\mathscr{E}^{\mathscr{C} \times \mathscr{D}}$の対象を\term{そうかんしゅ}{双関手}{bifunctor}と呼ぶ.
\end{definition}

\begin{proposition} \label{prop: adjunction_iff_hom_natural_iso}
$\mathscr{C}, \mathscr{D}$を局所小圏とし$F \colon \mathscr{C} \to \mathscr{D}, G \colon \mathscr{D} \to \mathscr{C}$を関手とする.\
$\Mor_\mathscr{D}(F(-), -) \colon \mathscr{C}^{\op} \times \mathscr{D} \to \mathbf{Set}, \Mor_\mathscr{C}(-, G(-)) \colon \mathscr{C}^{\op} \times \mathscr{D} \to \mathbf{Set}$を双関手とする.\
このとき次が成り立つ.
\begin{center}
...
\begin{proof}
自然同型$\Phi \colon \Mor_\mathscr{D}(F(-), -) \Rightarrow \Mor_\mathscr{C}(-, G(-))$が存在するとする.\
任意の$X \in \mathscr{C},\ Y \in \mathscr{D}$に対して$\eta_X \coloneq \Phi_{(X, F(X))}(\id_{F(X)}),\ \varepsilon_Y \coloneq \Phi^{-1}_{(G(Y), Y)}(\id_{G(Y)})$とする.

$X,X' \in \Ob(\mathscr{C}),\ Y,Y' \in \Ob(\mathscr{D}),\ u \in \Mor_{\mathscr{C}}(X', X),\ v \in \Mor_{\mathscr{D}}(Y, Y'),\ f \in \Mor_{\mathscr{D}}(F(X), Y),\ g \in \Mor_{\mathscr{C}}(X, G(Y))$とする.\
このとき$\Mor_{\mathscr{D}}(F(u), v)(f) = v \circ f \circ F(u)$かつ$\Mor_{\mathscr{C}}(u, G(v))(g) \coloneq G(v) \circ g \circ u$である.\
$\Phi$は自然同型なので次の図式が可換になる.
...
次の図式も任意の$X \in \Ob(\mathscr{C})$に対して可換になるので$\id_{F(X)} = \Phi^{-1}_{(X, F(X))}(\Phi_{(X, F(X))}(\id_{F(X)})) = \Phi^{-1}_{(X, F(X))}(\eta_X) = \Phi^{-1}_{(X, F(X))}(G(\id_{F(X)}) \circ \id_{GF(X)} \circ \eta_X) = \id_{F(X)} \circ \Phi^{-1}_{(GF(X), F(X))}(\id_{GF(X)}) \circ F(\eta_X) = \varepsilon_{F(X)} \circ F(\eta_X)$が成り立つ.\
つまり$\id_F = \varepsilon F \circ F \eta$である.
...
同様にして$\id_G = G \varepsilon \circ \eta G$が示せるので$(\eta, \varepsilon)$は随伴である.\
したがって随伴$F \dashv G$が存在する.

$F \dashv G$が存在するとする.\
$(\eta, \varepsilon) \coloneq F \dashv G$とすると$\eta \colon \id_{\mathscr{C}} \Rightarrow GF,\ \varepsilon \colon FG \Rightarrow \id_{\mathscr{D}}$であり$\varepsilon F \circ F \eta = \id_F$かつ$G \varepsilon \circ \eta G = \id_G$が成り立つ.\
$X \in \mathscr{C},\ Y \in \mathscr{D}$に対して$\Phi_{(X,Y)} \colon \Mor_{\mathscr{D}}(F(X), Y) \to \Mor_{\mathscr{C}}(X, G(Y))$を$\Phi_{(X,Y)}(f) \coloneq G(f) \circ \eta_X$で定め$\Psi_{(X,Y)} \colon \Mor_{\mathscr{C}}(X, G(Y)) \to \Mor_{\mathscr{D}}(F(X), Y)$を$\Psi_{(X,Y)}(g) \coloneq \varepsilon_Y \circ F(g)$で定める.

任意の$f \in \Mor_{\mathscr{D}}(F(X), Y)$に対して$\Psi_{(X,Y)}(\Phi_{(X,Y)}(f)) = \Psi_{(X,Y)}(G(f) \circ \eta_X) = \varepsilon_Y \circ F(G(f) \circ \eta_X) = \varepsilon_Y \circ FG(f) \circ (F \eta)_X$となる.\
$\varepsilon$は自然変換なので次の図式が可換になる.
...
任意の$g \in \Mor_{\mathscr{C}}(X, G(Y))$に対して$\Phi_{(X,Y)}(\Psi_{(X,Y)}(g)) = \Phi_{(X,Y)}(\varepsilon_Y \circ F(g)) = G(\varepsilon_Y \circ F(g)) \circ \eta_X = G \varepsilon_Y \circ GF(g) \circ \eta_X$となる.\
$\eta$は自然変換なので次の図式が可換になる.
...
任意の$X,X' \in \mathscr{C},\ Y, Y' \in \mathscr{D}$と任意の$u \in \Mor_{\mathscr{C}}(X', X),\ v \in \Mor_{\mathscr{D}}(Y, Y'),\ f \in \Mor_{\mathscr{D}}(F(X), Y)$に対して$\Phi_{(X',Y')}(v \circ f \circ F(u)) = G(v \circ f \circ F(u)) \circ \eta_{X'} = G(v) \circ G(f) \circ GF(u) \circ \eta_{X'}$となる.\
$\eta$は自然変換なので次の図式が可換になる.
...
局所小圏$\mathscr{C}$が小圏$\mathscr{J}$を形とする極限を持つことと定図式関手$\Delta \colon \mathscr{C} \to \mathscr{C}^{\mathscr{J}}$が右随伴関手を持つことは同値である.
\end{proposition}

\begin{proof}
任意の$F \in \mathscr{C}^{\mathscr{J}}$に対して$F$の極限$(L_F, \pi_F)$が存在するとする.\
$A \in \Ob(\mathscr{C})$と$F \in \mathscr{C}^{\mathscr{J}}$に対して$\Mor_{\mathscr{C}^{\mathscr{J}}}(\Delta(A), F)$は定関手$C_A$から$F$への自然変換全体の集合であり写像$\phi \colon \Mor_{\mathscr{C}^{\mathscr{J}}}(\Delta(A), F) \to \Cone(A,F)$を$\phi(\eta) \coloneq (A, \eta)$と定め写像$\psi \colon \Cone(A,F) \to \Mor_{\mathscr{C}^{\mathscr{J}}}(\Delta(A), F)$を$\psi(A, \eta) \coloneq \eta$と定めれば$\phi$と$\psi$は互いに逆写像となり$\Mor_{\mathscr{C}^{\mathscr{J}}}(\Delta(A), F) \cong \Cone(A,F)$である.

$\Phi_A \colon \Mor_{\mathscr{C}}(A, L_F) \to \Cone(A, F)$を$\Phi_A(h) \coloneq (A, \pi_F \circ \Delta(h))$で定める.\
任意の$(A, \alpha) \in \Cone(A, F)$に対して$\displaystyle (A, (A, \alpha)) \in \int_{\mathscr{C}^{\op}} \Cone(-, F)$である.\
$(L_F, (L_F, \pi_F))$は$\displaystyle \int_{\mathscr{C}^{\op}} \Cone(-, F)$の終対象なので射$h \colon (A, (A, \alpha)) \to (L_F, (L_F, \pi_F))$がただ1つ存在する.\
$\Delta(h) \colon C_A \Rightarrow C_{L_F}$と$\pi_F \colon C_{L_F} \Rightarrow F$の合成$\pi_F \circ \Delta(h) \colon C_A \Rightarrow F$は下の図式が可換になることから$\alpha$と等しい.\
つまり$\Phi_A(h) = (A, \alpha)$となる$h$がただ1つ存在するので$\Phi_A$は全単射である.
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	A & {L_F} & {F(X)} && {C_A(X)} & {C_{L_F}(X)} & {F(X)} \\
	&& {F(Y)} && {C_A(Y)} & {C_{L_F}(Y)} & {F(Y)}
	\arrow["h", from=1-1, to=1-2]
	\arrow["{\alpha_X}", curve={height=-18pt}, from=1-1, to=1-3]
	\arrow["{\alpha_Y}"', curve={height=12pt}, from=1-1, to=2-3]
	\arrow["{(\pi_F)_X}", from=1-2, to=1-3]
	\arrow["{(\pi_F)_Y}"', from=1-2, to=2-3]
	...
	\arrow["{(\pi_F)_X}", from=1-6, to=1-7]
	\arrow["{C_{L_F}(f)}"{description}, from=1-6, to=2-6]
	\arrow["{F(f)}", from=1-7, to=2-7]
	\arrow["{\Delta(h)_Y}", from=2-5, to=2-6]
	\arrow["{\alpha_Y}", curve={height=24pt}, from=2-5, to=2-7]
	\arrow["{(\pi_F)_Y}", from=2-6, to=2-7]
	\arrow[between={0.3}{0.6}, Rightarrow, nfold, 2tail reversed, from=0, to=1]
\end{tikzcd}\]
任意の$f \in \Mor_{\mathscr{C}}(A, B)$と$h \in \Mor_{\mathscr{C}}(B, L_F)$に対して$\Phi_A(h \circ f) = (A, \pi \circ \Delta(h \circ f)) = (A, \pi \circ \Delta(h) \circ \Delta(f)) = \Cone(f, F)(B, \pi \circ \Delta(h)) = \Cone(f, F)(\Phi_B(h))$なので次の図式が可換になる.
...
\[\begin{tikzcd}
	{\Mor_{\mathscr{C}}(B, L_F)} & {\Cone(B,F)} \\
	{\Mor_{\mathscr{C}}(A, L_F)} & {\Cone(A,F)}
...
したがって$\Phi \colon \Mor_{\mathscr{C}}(-,L_F) \Rightarrow \Cone(-,F)$は自然同型である.\
$F$は任意だったので$\Phi \colon \Mor_{\mathscr{C}}(-,\varprojlim_{\mathscr{J}}(-)) \Rightarrow \Cone(-,-)$は自然同型である.\
任意の$A \in \mathscr{C},\ F \in \mathscr{C}^{\mathscr{J}}$に対して$\Mor_{\mathscr{C}^{\mathscr{J}}}(\Delta(A), F) \cong \Cone(A,F)$だったので自然同型$\Mor_{\mathscr{C}}(-,\varprojlim_{\mathscr{J}}(-)) \Rightarrow \Mor_{\mathscr{C}^{\mathscr{J}}}(\Delta(-), -)$が存在する.

自然同型$\Phi \colon \Mor_{\mathscr{C}}(-,L_F) \Rightarrow \Mor_{\mathscr{C}^{\mathscr{J}}}(\Delta(-), F)$が存在すると仮定する.\
米田の補題(\cref{thm: yoneda_lemma})により全単射$\phi \colon \Nat(\Mor_{\mathscr{C}}(-, L_F), \Cone(-, F)) \to \Cone(L_F, F)$で$\phi(\alpha) = \alpha_{L_F}(\id_{L_F})$となるものが存在する.\
$\Phi \in \Nat(\Mor_{\mathscr{C}}(-, L_F), \Cone(-, F))$なので$\pi \coloneq \phi(\Phi)$とする.\
任意の$(A, (A, \alpha)) \in {\displaystyle \int_{\mathscr{C}^{\op}} \Cone(-, F)}$に対して$\Phi_A(h) = \alpha$となる$h \in \Mor_{\mathscr{C}}(A, {L_F})$がただ1つ存在する.\
$\Phi$は自然変換なので$\alpha = \Phi_A(h) = \Cone(h, F)(L_F, \pi) = (L_F, \pi \circ \Delta(h))$が成り立つ.\
よって$h$は$\alpha = \pi \circ \Delta(h)$を満たす$\displaystyle \int_{\mathscr{C}^{\op}} \Cone(-, F)$のただ1つの射$(A, (A, \alpha)) \to (L_F, (L_F, \pi))$なので$(L_F, (L_F, \pi))$は$\displaystyle \int_{\mathscr{C}^{\op}} \Cone(-, F)$の終対象である.
