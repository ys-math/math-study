...
\begin{definition}
$\mathscr{C}, \mathscr{D}$を圏とする.\ $X \in \mathscr{C}$に対して共変関手$\ev_X \colon \mathscr{D}^{C} \to \mathscr{D}$を$F \in \Ob(\mathscr{D}^{\mathscr{C}})$に対して$\ev_X(F) \coloneq F(X)$,\ $\eta \in \Mor_{\mathscr{D}^{\mathscr{C}}}$に対して$\ev_X(\eta) \coloneq \eta_X$で定める.\ これを\textbf{評価関手}(evaluation functor)という.
\end{definition}
