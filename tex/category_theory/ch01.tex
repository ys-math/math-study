...
$X \in \Ob(\mathbf{Set})$に対して$h^A(X) = \Mor_\mathbf{Set}(A, X)$で定め$f \in \Mor(\mathbf{Set})$に対して$h^A(f) \colon h^A(\dom(f)) \to h^A(\cod(f))$を$h^A(f)(g) \coloneq f \circ g$で定めると$h^A \colon \mathbf{Set} \to \mathbf{Set}$は共変関手である.
\end{example}

\begin{definition} \label{def: NaturalTransformation}
$F,G$を圏$\mathscr{C}$から圏$\mathscr{D}$への共変関手とする.\ $\mathscr{D}$の射の族$\eta = (\eta_X \colon F(X) \to G(X))_{X \in \Ob(\mathscr{C})}$は任意の$f \in \Mor(\mathscr{C})$に対して次の図式を可換にするとき\textbf{自然変換}(natural transformation)であるという.\ 特に$\eta$が自然変換で任意の$X \in \Ob(\mathscr{C})$に対して$\eta_X$が全単射であるとき$\eta$は$F$から$G$への\textbf{自然同型}(natural isomorphism)であるといい$\eta \colon F \Rightarrow G$と書く.\ $F$から$G$への自然変換全体の集まりを$\Nat(F, G)$と書く.
...
\[\begin{tikzcd}
	...
	\arrow["{\eta_{\dom(f)}}", from=2-1, to=2-2]
\end{tikzcd}\]
