...
$\mathscr{C}, \mathscr{D}$を圏とする.\ 対象を関手$F \colon \mathscr{C} \to \mathscr{D}$とし関手$F \colon \mathscr{C} \to \mathscr{D}$から関手$G \colon \mathscr{C} \to \mathscr{D}$への射を自然変換$\eta \colon F \Rightarrow G$とする.\ $\eta \colon F \Rightarrow G$と$\zeta \colon G \Rightarrow H$の合成を$X \in \Ob(\mathscr{C}^{\mathscr{D}})$に対して$(\zeta \circ \eta)_X \coloneq \zeta_X \circ \eta_X$,\ $F$の恒等射$\id_F$を$X \in \Ob(\mathscr{C}^{\mathscr{D}})$に対して$(\id_F)_X \coloneq \id_{F(X)}$で定めることで圏$\mathscr{D}^{\mathscr{C}}$が定まる.\ これを\textbf{関手圏}(functor category)という.
\end{definition}