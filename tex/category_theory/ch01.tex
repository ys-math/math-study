...
\begin{definition}
$\mathscr{C}, \mathscr{D}$を圏とする.\ 対象を関手$F \colon \mathscr{C} \to \mathscr{D}$とし関手$F \colon \mathscr{C} \to \mathscr{D}$から関手$G \colon \mathscr{C} \to \mathscr{D}$への射を自然変換$\eta \colon F \Rightarrow G$とすることで圏$\mathscr{D}^{\mathscr{C}}$が定まる.\ これを\textbf{関手圏}(functor category)という.
\end{definition}