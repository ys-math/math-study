...
$\mathscr{J}, \mathscr{C}$を圏とし$F \colon \mathscr{J} \to \mathscr{C}$を関手とする.\ $F$を\term{かたち}{形}{shape}$\mathscr{J}$の\term{ずしき}{図式}{diagram}と呼ぶ.\ $A \in \Ob(\mathscr{C})$とし$C_A \colon \mathscr{J} \to \mathscr{C}$を定関手とする.\ $A$と自然変換$\eta \colon C_A \Rightarrow F$の組$(A, \eta)$を$A$を\term{ちょうてん}{頂点}{apex}とする$F$への\term{すい}{錐}{cone}という.
