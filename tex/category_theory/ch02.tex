...
\begin{proof}
$\psi(u)$が自然同型であるとき任意の$X \in \Ob(\mathscr{C})$に対して$\psi(u)_X$は全単射である.\ $B \in \Ob(\mathscr{C}), x \in F(B)$とする.\ $f \coloneq \psi(u)_B^{-1}(x)$とすると$F(f)(u) = \psi(u)_B(f) = \psi(u)_B(\psi(u)_B^{-1}(x)) = x$となり$\psi(u)_B$は全単射なのでこのような$f$はただ1つだから$u$は$F$の普遍元である.

$u$が$F$の普遍元であるとき任意の$B \in \Ob(\mathscr{C}), x \in F(B)$に対して$F(f)(u) = x$を満たす$f \in \Mor_\mathscr{C}(A, B)$がただ1つ存在する.\ $B \in \Ob(\mathscr{C})$に対して$g_B \colon F(B) \to h^A(B)$を$g_B(x) \coloneq f$と定めれば$\psi(u)_B(g_B(x)) = \psi(u)_B(f) = F(f)(u) = x$であり$g_B(\psi(u)_B(f)) = g_B(F(f)(u)) = f$となり$\psi(u)_B$は全単射である.
\end{proof}
