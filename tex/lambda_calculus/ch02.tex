...
\begin{definition}
$\mathbb{B}$を空でない集合とし$\mathbb{B}$の元を\term{きていがた}{基底型}{base type}と呼ぶことにする.\
$\mathbb{T}$を次の条件を満たす最小の集合とする.
\begin{itemize}
    \item $\mathbb{B} \subset \mathbb{T}$
    \item ${^{\forall}}A,B \in \mathbb{T}, (A \to B) \in \mathbb{T}$
\end{itemize}
$\mathbb{T}$の元を\term{かた}{型}{type}という.
BNF記法を用いて型は$\textrm{type} ::= \iota \mid (\textrm{type} \to \textrm{type})$と定義できる.\
ただし$\iota \in \mathbb{B}$である.
\begin{center}
