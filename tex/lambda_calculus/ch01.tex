...
\begin{definition}
$V$を可算集合とし$V$の元を\term{へんすう}{変数}{variable}と呼ぶことにする.\
$\Lambda$を次の条件を満たす最小の集合とする.
\begin{itemize}
    \item $V \subset \Lambda$
    \item ${^{\forall}}M, N \in \Lambda, (M N) \in \Lambda$
    \item ${^{\forall}}x \in V, {^{\forall}}M \in \Lambda, (\lambda x.M) \in \Lambda$
\end{itemize}
$\Lambda$の元を\term[lambda-term]{らむだこう}{$\lambda$項}{$\lambda$-term}と呼ぶ.
\end{definition}
...
\begin{definition}
$M \in \Lambda$とし$x, z \in V, y \notin \textrm{Var}(M)$とする.\
$M \langle x \mapsto y\rangle$を次のようにして帰納的に定める.
\begin{itemize}
...
\end{center}
$\to_\alpha$を\term[alpha-step]{あるふぁすてっぷ}{$\alpha$ステップ}{$\alpha$-step}と呼ぶ.\
つまり$x,y \in V, M \in \Lambda, y \notin \textrm{Ver}(M)$に対して
\begin{center}
...
\end{itemize}
$\equiv_\alpha$を\term[alpha-conversion]{あるふぁへんかん}{$\alpha$変換}{$\alpha$-conversion}という.
\end{definition}
...
\end{center}
$\to_\beta$を\term[one-step beta-reduction]{いちすてっぷべーたかんやく}{1ステップ$\beta$簡約}{one-step $\beta$-reduction}という.
\end{definition}
...
$\twoheadrightarrow_\beta$を\term[beta-reduction]{べーたかんやく}{$\beta$簡約}{$\beta$-reduction}という.
\end{definition}
...
\begin{itemize}
    ...
    \item $=_\beta$は同値関係.
\end{itemize}
$=_\beta$を\term[beta-conversion]{べーたへんかん}{$\beta$変換}{$\beta$-conversion}という.
\end{definition}
